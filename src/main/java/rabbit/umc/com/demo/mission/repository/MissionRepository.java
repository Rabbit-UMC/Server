...
public interface MissionRepository extends JpaRepository<Mission,Long> {
    ...
    @Query("select m from Mission m left join fetch m.category where m.category.id = :missionCategoryId and m.isOpen = :isOpen and m.endAt >= :now and m.status = :active order by m.startAt")
...
//                                            PageRequest pageRequest);
    List<Mission> findAllByStatusAndEndAtAfterAndIsOpenOrderByStartAt(Status active, LocalDateTime now, int isOpen, PageRequest pageRequest);
